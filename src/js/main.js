...
$(function() {
  // ajax
  $.getJSON('https://www.lionet.xyz/wanwushuo/php/', function(data) {
    // 行业快讯
  ...
  // navbar
  $('#nav li').not('.head').click(function() {
  ...
  function updateNavBar(data) {
    ...
    if(currentLiLength < data.length) {
      ...
      for(let i=0; i<currentLiLength-data.length; i++) {
        target.find('li').not('.tip').eq(currentLiLength-i-1).remove();
      }
  ...
  $('header .search-input span').click(function() {
    ...
    window.open(`https://www.qianjia.com/Search.html?type=1&value=${searchContent}`);
  });
